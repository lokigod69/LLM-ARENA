...
| `callUnifiedOpenRouter` | `qwen-plus`, `qwen3-max` | `POST https://openrouter.ai/api/v1/chat/completions` | `Authorization: Bearer ${OPENROUTER_API_KEY}` + `HTTP-Referer`, `X-Title` headers | Adds `provider` metadata from config comment, standard Chat Completions payload. |
